...
2026-09-15:
	...
	Lab DNS records changed from `address=` / `ptr-record=` pairs to `host-record` lines, which fixed NXDOMAIN answers to AAAA queries (Linear ISA-6). The original config is saved as `/etc/dnsmasq.conf.bak-rangelab`. See [[dnsmasq]] and [[Troubleshooting]] (2026-09-15).
